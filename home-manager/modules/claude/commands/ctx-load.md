...
1. Find and read `PR.md`:
   * Check if `PR.md` at repo root exists - if symlink, follow it to actual location
   ...
   * Load any sub-files (`PR-<phase-name>.md` in same directory) if planning to work on them
...
3. Synthesize and propose next steps:
   ...
   * Propose `PR.md` / `PR-<phase-name>.md` updates if missing information
