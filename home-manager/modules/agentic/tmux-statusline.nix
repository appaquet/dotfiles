    ...
    (pkgs.writeShellScriptBin "tmux-statusline" ''
      ...
      case "$#:$1" in
        ...
        1:clear)
          ...
          ;;
        *)
          fail "usage: tmux-statusline set <indicator> | tmux-statusline clear"
          ;;
      esac

      # Valid lifecycle calls outside tmux intentionally have no observable effect.
      [ -n "$TMUX" ] || exit 0
      ...
      if [ -n "$TMUX_WINDOW_ID" ]; then
        ...
        target="$("$tmux" display-message -p '#{window_id}')" \
          || fail "could not resolve the invoking tmux window; set TMUX_WINDOW_ID explicitly"
      fi
