// Dotfiles-owned modified copy of Herdr 0.8.2's Pi integration (integration version 8).
// Upstream: https://github.com/herdrdev/herdr/blob/v0.8.2/src/integration/assets/pi/herdr-agent-state.ts
...
export default function (pi) {
  ...
  let sessionIdentityReported = false;

  function desiredState() {
    if (blockedCount > 0) {
      return { state: "blocked" as const, message: blockedMessage };
    }
    if (agentActive || activeSubagentIds.size > 0) {
  ...
  pi.events.on("subagents:created", (event) => {
    updateSubagentActivity(event, "created");
  });

  pi.events.on("subagents:started", (event) => {
    updateSubagentActivity(event, "started");
  });

  pi.events.on("subagents:completed", (event) => {
    updateSubagentActivity(event, "terminal");
  });
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    rootSession = true;
    sessionIdentityReported = false;
    updateSessionRef(ctx);
    await reportSession(event?.reason);
    sessionIdentityReported = true;
  ...
  function updateSubagentActivity(
    event: unknown,
    lifecycle: "created" | "started" | "terminal",
  ): void {
    ...
    if (lifecycle === "terminal") {
      activeSubagentIds.delete(id);
      terminalSubagentIds.add(id);
    } else if (lifecycle === "started") {
      terminalSubagentIds.delete(id);
      activeSubagentIds.add(id);
    } else if (!terminalSubagentIds.has(id)) {
      activeSubagentIds.add(id);
    }
    publishState();
  }
