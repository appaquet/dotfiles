    ...
    { scope }:
    {
      content = ''
        ...
        * `jj log` has no Git `-S` pickaxe or `-f` follow flag; use path arguments and inspect history with supported revsets
        ...
        | Purpose | Command |
        |---------|---------|
        | Finalize current changes and create a new empty `@` | `jj commit -m "private: agent: description"` |
        | Finalize selected files; remaining files move to the new `@` | `jj commit -m "private: agent: description" <files...>` |
        | Create a child of the current or named revision | `jj new [<rev>] -m "private: agent: description"` |
        | Describe the current change | `jj describe -m "private: agent: description"` |
        | Move current content into its parent, keeping the parent's message | `jj squash -u [<files...>]` |
        | Move current content into its parent and replace the parent's message | `jj squash -m "private: agent: description" [<files...>]` |
        | Split selected files into the original change | `jj split -m "private: agent: description" <files...>` |
        ...
        * Empty `@` -> `jj describe -m "private: agent: description"`
        * `@` has content -> `jj new -m "private: agent: description"`
        ...
        * Resolve a conflicted revision in a child created by `jj new <change_id>`, inspect the resolution with `jj diff`, then apply it with `jj squash -u`; do not `jj edit` the conflicted revision
        ...
        Prefix agent change descriptions with `private: agent: ` and keep the remainder near 10 words. AP's `private()` policy prevents those changes from being pushed, and `agent()` selects them. Never let a message editor open: pass `-m` when assigning a description, or `-u` when squashing while keeping the destination message. Undescribed scratch children such as conflict-resolution changes may use `jj new <rev>`.
