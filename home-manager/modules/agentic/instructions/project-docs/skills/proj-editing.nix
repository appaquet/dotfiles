  ...
  nixantic.sources.project-docs.skills."proj-editing" = {
    ...
    main = {
      ...
      content = ''
        ...
        - Phase docs (`proj/NN-<phas>.md`) created mainly via `ctx-plan`, but can be created adhoc on user request. If phase doc unrelated to new work, ask user if split. Updated on task complete, on `ctx-save` call, significant new info, uncertainties, decisions, insights, etc.
