...
let
  ...
  # type. Source paths below refer to normalized source branches; the live
  # directory tree is imported from source-sets/<feature-or-domain>/ before these
  # constructors run. Scope-injected fields are listed for awareness (authors
  # should not set them). Fields consumed by scope.nix before the constructor runs
  # are marked as scope-consumed.
